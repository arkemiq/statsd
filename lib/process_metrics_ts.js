...
var process_metrics = function (metrics, flushInterval, ts, flushCallback) {
    ...
    var aggregation = metrics.aggregation || 'avg';

    // counters = _.clone(counters, true);
    for (key in counters) {
      var timeseries = counters[key];

      for (var time in timeseries) {
        sum = _.reduce(timeseries[time], function(sum, num) { return sum + num; });
        t = {};
        t[time] = sum / (flushInterval / 1000);
        counters[key][time] = sum;
        counter_rates[key] === undefined ? counter_rates[key] = {} : null;
        _.merge(counter_rates[key], t);
      }
    }
    ...
    for (key in gauges) {
      ...
      for (var time in timeseries) {
        var values = timeseries[time];
        var value = 0;
        switch (aggregation) {
          case 'avg':
            len = values.length || 1;  
            sum = _.reduce(values, function(sum, num) { return sum + num; });
            value = sum / len;
            break;
          case 'max':
            value = _.max(values);
        ...
        gauges[key][time] = value;
      }
    }

    for (key in timers) {

      var timeseries = timers[key];

      for (var time in timeseries) {
        var current_timer_data = {};
        if (timers[key][time].length > 0) {
          timer_data[key] = {};

          timer_counters[key][time] = _.reduce(timer_counters[key][time], function(sum, num) { return sum + num;})

          var values = timers[key][time].sort(function (a,b) { return a-b; });
          ...
          current_timer_data["count"] = timer_counters[key][time];
          current_timer_data["count_ps"] = timer_counters[key][time] / (flushInterval / 1000);
        ...
        timer_data[key][time] = current_timer_data;
      }
