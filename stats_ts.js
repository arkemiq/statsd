...
var dgram  = require('dgram')
  ...
  , set = require('./lib/set')
  , pm = require('./lib/process_metrics_ts')
...
function flushMetrics() {
  ...
  counters = aggregate_metrics(counters, time_stamp);
  gauges = aggregate_metrics(gauges, time_stamp);
  timers = aggregate_metrics(timers, time_stamp);
  timer_counters = aggregate_metrics(timer_counters, time_stamp);
  timer_data = aggregate_metrics(timer_data, time_stamp);
  ...
  backendEvents.once('flush', function clear_metrics(ts, metrics) {
    ...
    for (var timer_key in metrics.timers) {
      if (conf.deleteTimers) {
        ...
        metrics.timer_counters[timer_key] = [];
     }
    }
...
config.configFile(process.argv[2], function (config, oldConfig) {
  ...
  config.host = host = os.hostname().replace(/\./g, '-') + '-' + config.port;
  bad_lines_seen   = prefixStats + "." + host + ".bad_lines_seen";
  packets_received = prefixStats + "." + host + ".packets_received";
  timestamp_lag_namespace = prefixStats + "." + host + ".timestamp_lag";
  ...
  if (server === undefined) {
    ...
    server = dgram.createSocket(udp_version, function (msg, rinfo) {
      ...
      counters[packets_received].push([1, now]);
      ...
      for (var midx in metrics) {
        ...
        var tsbits = metrics[midx].toString().split(' ');
        var msg = tsbits[0];
        var ts = tsbits[1] !== undefined ? 
                    (tsbits[1].length > 12 ? Math.round(tsbits[1]/1000) : +tsbits[1]) 
        ...
        for (var i = 0; i < bits.length; i++) {
          ...
          if (!helpers.is_valid_packet(fields)) {
              ...
              counters[bad_lines_seen].push([1, now]);
          ...
          if (metric_type === "ms") {
            ...
            timers[key].push([Number(fields[0] || 0), ts]);
            timer_counters[key].push([1 / sampleRate, ts]);
          } else if (metric_type === "g") {
            if (! gauges[key]) {
              gauges[key] = [];
            }
            gauges[key].push([Number(fields[0] || 0), ts]);
            ...
            counters[key].push([Number(fields[0] || 1) * (1 / sampleRate), ts])
          }
        }
      }
